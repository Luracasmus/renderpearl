...
void main() {
	...
	if (dot(model.xz, model.xz) < farSquared) {
		...
		#ifdef ENTITY_COLOR
			v.tint = mix(color, f16vec3(entityColor.rgb), float16_t(entityColor.a));
		#endif
		v.tint = vec3(color);
		...
		immut float16_t chebyshev_dist = max3(abs_pe.x, abs_pe.y, abs_pe.z);
		...
		#ifdef HAND
			v.emission = uint(handLightLevel);

			#if HAND_LIGHT
				if (handLightLevel > 0) {
					immut uvec3 scaled_color = uvec3(fma(linear(vaColor.rgb * textureLod(gtexture, mix(v.coord, mc_midTexCoord, 0.5), 0.0).rgb), vec3(255.0), vec3(0.5)));
			...
			v.light.x = min(fma(float16_t(emission), float16_t(0.3), max(v.light.x, emission)), float16_t(1.0));

			v.emission = uint(fma(emission, float16_t(15.0), float16_t(0.5))); // bring into 4-bit-representable range and round
					...
					if (uint8_t(pcg(seed.x + pcg(seed.y + pcg(seed.z)))) % (uint8_t(1u) << uint8_t(min(7.0, fma(
						...
						index.data[i] = bitfieldInsert(
							bitfieldInsert(
								...
								uint(emission + float16_t(0.5)), 27, 4 // intensity
							),
		...
		if (chebyshev_dist < float16_t(shadowDistance * shadowDistanceRenderMul)) {
			...
			#else
				immut f16vec3 w_normal = f16vec3(tbn_comp.normal);
			#endif
