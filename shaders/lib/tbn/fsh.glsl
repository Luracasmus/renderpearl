...
mat3 get_tbn() {
	immut f16vec2 octa_normal = f16vec2(unpackFloat2x16(v_tbn.half2x16_octa_normal_and_tangent.x));
	immut vec3 normal = vec3(normalize(octa_decode(octa_normal)));

	immut f16vec2 octa_tangent = f16vec2(unpackFloat2x16(v_tbn.half2x16_octa_normal_and_tangent.y));
