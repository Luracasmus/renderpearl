VertexData {
	...
	layout(location = 1, component = 0) flat uint uint4_bool1_unorm11_float16_emission_handedness_alpha_luma;
		...
		#ifdef TRANSLUCENT
			layout(location = 3, component = 0) vec3 light; // RGB
		...
		layout(location = 1, component = 1) flat uint unorm4x8_tint_zero;
			...
			#ifndef NO_NORMAL
				layout(location = 1, component = 2) flat uint snorm4x8_octa_tangent_normal;

				#if NORMALS != 2 && !(NORMALS == 1 && defined MC_NORMAL_MAP)
					layout(location = 1, component = 3) flat uint unorm2x16_mid_coord;
					layout(location = 4, component = 0) flat uint uint2x16_face_tex_size;
				#endif
			#endif
		#else
