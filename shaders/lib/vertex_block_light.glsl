	...
	if (light.x > float16_t(0.0) && chebyshev_dist < float16_t(LL_DIST)) {
		...
		for (uint16_t i = uint16_t(0u); i < index_len; ++i) {
			...
			if (mhtn_dist <= intensity) {
				...
				immut float16_t falloff = light_data >= 0x80000000u ? rcp_dist_light : float16_t(1.0) / sq_dist_light; // todo!() this is WRONG, check deferred
				...
				immut float16_t light_diffuse = face_n_dot_l > float16_t(0.0) ? face_n_dot_l / float16_t(PI) : float16_t(IND_BL + 0.04); // add IND_BL for fake "GI", with additional (+0.04, just picked at random) term not found in the solid version, to also fake light spreading through the material
