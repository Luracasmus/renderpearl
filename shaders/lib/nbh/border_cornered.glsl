...

{
	...
	if (local.x == int8_t(0) && !down) { // left && !down
		...
		BORDER_OP(ivec2(0, 1))
	} else if (local.x == int8_t(1) && local.y >= int8_t(gl_WorkGroupSize.x - 3u)) { // 1 step inside lower left corner || 1 step above that
		BORDER_OP(ivec2(-2, 2))
	} else if (local.y == int8_t(1) && local.x <= int8_t(2)) { // 1 step inside upper left corner || 1 step to the right of that
		BORDER_OP(ivec2(-2, -2))
	} else if (local.x == int8_t(gl_WorkGroupSize.x - 2u) && local.y <= int8_t(2)) { // 1 step inside upper right corner || 1 step below that
		BORDER_OP(ivec2(2, -2))
	} else if (local.y == int8_t(gl_WorkGroupSize.x - 2u) && local.x >= int8_t(gl_WorkGroupSize.x - 3u)) { // 1 step inside lower right corner || 1 step to the left of that
		BORDER_OP(ivec2(2, 2))
	} else NON_BORDER_OP
}